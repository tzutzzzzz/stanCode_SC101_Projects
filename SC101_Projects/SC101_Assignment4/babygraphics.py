...
def draw_names(canvas, name_data, lookup_names):
    ...
    for name in lookup_names:
        x_end = y_end = 0
        color = COLORS[color_index % len(lookup_names)]
        color_index += 1
        if name in name_data:
            for i in range(len(YEARS)):
                ...
                if year in name_data[name]:
                    y = int(name_data[name][year]) / 1000 * (CANVAS_HEIGHT - GRAPH_MARGIN_SIZE * 2) + GRAPH_MARGIN_SIZE
                ...
                if i > 0:
                    canvas.create_line(x, y, x_end, y_end, width=LINE_WIDTH, fill=color)
                x_end = x
                y_end = y
